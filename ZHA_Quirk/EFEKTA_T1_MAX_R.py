# EFEKTA_T1_MAX_R.py
# Quirk for EFEKTA_T1_MAX_R (Temperature smart monitor with voltage detector. Simple Thermostat.)
...
class TempResolutionEnum(IntEnum):
    BIT_9 = 9
    BIT_10 = 10
    BIT_11 = 11
    BIT_12 = 12
...
class TimeExt(Time, CustomCluster):
    class AttributeDefs(Time.AttributeDefs):
        lifetime: Final = ZCLAttributeDef(id=0x0007, type=t.uint32_t, access="r")
...
class TempMeasurement(TemperatureMeasurement, CustomCluster):
    class AttributeDefs(TemperatureMeasurement.AttributeDefs):
        ...
        temp_resolution: Final = ZCLAttributeDef(id=0x0520, type=t.uint8_t, access="rw")

(
    QuirkBuilder(EFEKTA, "EFEKTA_T1_MAX_R")
    ...
    .sensor(
        TimeExt.AttributeDefs.lifetime.name,
        TimeExt.cluster_id,
        endpoint_id=1,
    ...
    .enum(
        TempMeasurement.AttributeDefs.temp_resolution.name,
        TempResolutionEnum,
        TempMeasurement.cluster_id,
        endpoint_id=1,
        translation_key="temp_resolution",
        fallback_name="T Resolution",
        unique_id_suffix="temp_resolution",
        entity_type=EntityType.CONFIG,
        entity_platform=EntityPlatform.SELECT,
    )
    .switch(
